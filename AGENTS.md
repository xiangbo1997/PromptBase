...
## 4. Frontend Dev Cache Recovery

For this repository, Next.js dev cache corruption is a known local failure mode after frontend edits.

- Prevention rule: do not run `next build` for `apps/web` while `next dev` is actively serving the same workspace, because both commands write to `apps/web/.next` and can corrupt the dev runtime chunks. If a production build verification is required, stop the dev server first or restart it after the build.
- Symptom pattern: browser or terminal shows `Server Error` with missing chunks such as `Cannot find module './769.js'`, `Cannot find module './vendor-chunks/@tanstack.js'`, or similar errors originating from `apps/web/.next/server/webpack-runtime.js`.
- Interpretation: treat this as a broken `.next` runtime/cache state first, not as proof that the latest code change is logically wrong.
- Required recovery flow:
  1. Stop the current dev process serving `apps/web`.
  2. Move `apps/web/.next` aside to a timestamped backup directory such as `apps/web/.next.broken-YYYYMMDD-HHMM`.
  3. Restart the dev stack so Next.js rebuilds `.next` from scratch.
  4. Re-verify `http://127.0.0.1:3008/login` returns `200` and `http://127.0.0.1:3001/health` returns healthy JSON before concluding the edit is broken.
- After any frontend change, if this symptom appears, execute the recovery flow before further code edits.
